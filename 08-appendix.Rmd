...
\begin{equation}
   ...
   \end{split}
\end{equation}
where $x \in \mathbb{R}^n$ the state, $u \in \mathbb{R}^m$ the control input, $y \in \mathbb{R}^p$ the output, and and $A,B,C,D$ are constant matrices with proper sizes. If we know the initial state $x(0)$ and the control inputs $u(t)$ over a period $t \in [0, t_1]$, $(x(t), y(t))$ can be expressed as
\begin{equation}
    (\#eq:app-lti-xy)
   \begin{split}
...
To study the inner structure of linear systems, two important properties should be considered: controllability and observability. In the following analysis, we will see that they are actually dual concepts. Their definitions [@chen1984book-linear] are shown below:
...
The LTI system \@ref(eq:app-linear-system) or the pair $(C, A)$ is observable, if for any unknown initial state $x(0)$, there exists a finite $t_1 > 0$, such that knowing $y$ and $u$ over $[0, t_1]$ suffices to determine $x(0)$.
:::
:::

Sometimes we want to analyze the system \@ref(eq:app-linear-system) under another basis, i.e., $z = T x$, where the similarity transformation matrix $T$ is nonsingular. Define $A' = TAT^{-1}, B' = PB, C' = CT^{-1}, D' = D$, we get 
\begin{equation*}
   \begin{split}
      \dot{z} = A' z + B' u \\ 
      y = C' z + D' u
   \end{split}
\end{equation*}
Since the similar transformation only changes system's basis for description, physical properties like controllability and observability will not change.

### Cayley Hamilton Theorem

In the analysis of controllability and observability, Cayley Hamilton Theorem lays the foundation. The theory statement and an elegant proof of it are shown below. Some useful corollaries and their proofs are also given. 

::: {.theorembox}
::: {.theorem #cayham name="Cayley Hamilton"}
Let $A \in \mathbb{C}^{n \times n}$ and denote 
$$
   \text{det}(\lambda I - A) = \lambda^n + a_1 \lambda^{n-1} + \dots + a_n
$$
Then
$$
...
::: {.proofbox}
::: {.proof}
Define the adjugate of $\lambda I - A$ as 
$$
...
\begin{equation*}
   \begin{split}
      ...
      & = \lambda^n I + \sum_{i=1}^{n-1} \lambda^i a_{n-i} I + a_n I  
   \end{split}
\end{equation*}

Since $\lambda$ can be arbitrarily set, we must have
\begin{equation*}
   \begin{split}
      B_{n-1} & = I \\
...
Here are some corollaries of Cayley Hamilton Theorem. 
...
\begin{equation*}
   ...
   \end{bmatrix}
\end{equation*}
Thus, $\text{rank}\begin{bmatrix}
   \bar{B}_c & \bar{A}_c \bar{B}_c & \bar{A}_c^2 \bar{B}_c & \dots & \bar{A}_c^{k_1 - 1} \bar{B}_c & \dots & \bar{A}_c^{n - 1} \bar{B}_c
\end{bmatrix} = k_1$. By Corollary \@ref(cor:cayham-2), $\text{rank}\bar{\mathcal{C}} = k_1$.
:::
:::
...
There are a few equivalent statements to express a LTS system's controllability that one should be familiar with:
...
7. If, in addition, all eigenvalues of $A$ have negative parts, then the unique solution of 
$$ 
   A W_c + W_c A^* = -B B^* 
$$
...
\end{equation*}
is nonsingular for all $t >0$. The latter is exactly the definition of $(A^*, C^*)$'s $W_c(t)$.
:::
:::
...
$\lambda$ is a mode of an LTI system, if it is an eigenvalue of $A$. The mode $\lambda$ is said to be stable if $\text{Re}\lambda < 0$, is controllable (observable) if $x^* B \ne 0$ ($C x \ne 0$) for all left (right) eigenvectors of $A$ associated with $\lambda$. Otherwise, the mode is said to be uncontrollable (unobservable).
:::
:::

With the concept of system mode, the fifth definition of controllability \@ref(thm:lticontrollable) (observability \@ref(thm:ltiobservable)) can be restated as: an LTI system is controllable (observable) if and only if all modes are controllable (observable). Stabilizability (detectability) is defined similarly via loosening part of controllability (observability)'s conditions.
...
\begin{equation*}
   \sum_{i=1}^{I} \left\{
      ...
      + \xi_{i,K_i} \cdot (\eta_i - \lambda) \cdot v_{i,K_i}^*
   \right\}
\end{equation*}
