...
::: {.definitionbox}
::: {.definition #lti-controllable name="Controllability"}
...
::: {.definitionbox}
::: {.definition #lti-observable name="Observability"}
...
$$
   ...
   \end{bmatrix}
$$
where $\bar{A}_c \in \mathbb{C}^{k_1 \times k_1}, \bar{B}_c \in \mathbb{C}^{k_1 \times m}$. 
:::
:::
...
Although controllability and observability seemingly have no direct connections from their definitions \@ref(def:lti-controllable) and \@ref(def:lti-observable), the following theorem [@chen1984book-linear] states their tight relations:
