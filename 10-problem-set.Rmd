...
::: {.exercisebox}
::: {.exercise #energy-pump name="Energy pumping of Clipped LQR Controller"}
In Exercise \@ref(exr:pendulum-lqr-clipped-roa), we have studied the region of attraction of Clipped LQR Controller. In this exercise, we are going to swing up the pendulum to a stabilizable initial state (the blue circle region in Exercise \@ref(exr:pendulum-lqr-clipped-roa)).
...
With the same parameter setting as in the previous exercise, simulate the system under the saturated controller with initial state $(x_1,x_2)=(\pi,1).$ Will the state hit the candidate ROA $\Omega_\rho$ with $\rho=2$? If yes, stop the system once it hits the region, and draw the trajectory on the Figure similar to Fig. \@ref(fig:pendulum-sat-control-random-samples-certificate).

With the above exercises, we can swing the pendulum to a stabilizable state, and then switch to the LQR phase as implemented in Exercise \@ref(exr:pendulum-lqr-clipped-roa).
