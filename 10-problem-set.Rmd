...
In this question, we will study the cart-pole system that we saw the video of human-controlled version in our first lecture. The task is to balance a pendulum on a cart by horizontally moving the cart. See this video (https://www.youtube.com/watch?v=Bzq96V1yN5k) for an actual robotic implementation. 

```{r cart-pole, out.width='100%', fig.show='hold', fig.cap='Illustration of cart-pole problem', fig.align='center', echo=FALSE}
...
a. Denote the mass of cart and pole as $m_c$ and $m_p$, respectively. 
...
d. (Bonus) Will a linear controller (i.e., $f$ is linear in $\mathbf{x}$) be a good controller? Why or why not? The answer might depend on whether the end point of the pole is above the horizonal line.
:::
:::
# Acknowledgement {-}