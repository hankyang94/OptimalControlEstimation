...
The dynamics of a planar two-link manipulator is given as 
\begin{equation}
...
$$
where $\Lambda \in \mathbb{S}^n_{++}$ is a user-chosen positive definite matrix (in general we want $-\Lambda$ to be Hurwitz). In this case, $s \rightarrow 0$ implies $\tilde{q} \rightarrow 0$ as $t \rightarrow \infty$. Choosing the control law 
\begin{equation}
...
H s + K_D s = 0.
$$
Therefore, designing $K_D = \lambda H$ with $\lambda > 0$ guarantees the convergence of $s$ to zero.
...
$$
\tau = Y (q,\dot{q},\dot{q}_r,\ddot{q}_r) a - K_D s
$$
where $a \in \mathbb{R}^m$ contains all the parameters and $Y \in \mathbb{R}^{n \times m}$ is the matrix that collects all the coeffients of $a$ (i.e., $Ya = H \ddot{q}_r + C \dot{q} + g(q)$). As a result, the adapative control law is naturally
\begin{equation}
\tau = Y \hat{a} - K_D s,
(\#eq:ac-mrac-manipulator-controller-unknownparam)
\end{equation}
with $\hat{a}$ the time-varying parameter that we wish to adapt.

**Adaptation law and tracking convergence**. With the adapative control law \@ref(eq:ac-mrac-manipulator-controller-unknownparam), the closed-loop dynamics becomes 
\begin{align}
...
:::
::: {.proof}
<span style="color:red">TBD or add reference.</span>
:::
...
&= s^T H (\ddot{q} - \ddot{q}_r) + \frac{1}{2} s^T \dot{H} s + \tilde{a}^T \Gamma^{-1} \dot{\tilde{a}} \\
&= s^T (\tau - H \ddot{q}_r - C \dot{q} - g) + \frac{1}{2} s^T \dot{H} s + \tilde{a}^T \Gamma^{-1} \dot{\tilde{a}} \\
&= s^T (\tau - H \ddot{q}_r - C (s + \dot{q}_r) - g) + \frac{1}{2} s^T \dot{H} s + \tilde{a}^T \Gamma^{-1} \dot{\tilde{a}} \\
&= s^T (\tau - H \ddot{q}_r - C \dot{q}_r - g) + \frac{1}{2} s^T (\dot{H}- 2C)s + \tilde{a}^T \Gamma^{-1} \dot{\tilde{a}} \\
&= s^T (\tau - H \ddot{q}_r - C \dot{q}_r - g) + \tilde{a}^T \Gamma^{-1} \dot{\tilde{a}}. 
\end{align}
Further, with the control law \@ref(eq:ac-mrac-manipulator-controller-unknownparam), we have
\begin{align}
\dot{V} &= s^T ( Y\tilde{a} - K_D s + Y a - H \ddot{q}_r - C \dot{q}_r - g) + \tilde{a}^T \Gamma^{-1} \dot{\tilde{a}} \\
& = s^T (Y\tilde{a} - K_D s + Cs ) + \tilde{a}^T \Gamma^{-1} \dot{\tilde{a}}
\end{align}
