...
\protect\hypertarget{thm:lyapunovglobalstability}{}\label{thm:lyapunovglobalstability}For the autonomous system \eqref{eq:autonomous-system}, suppose there exists a scalar function \(V(x)\) with continuous first order derivatives such that
...
\]
\[
    V(z) > 0 \quad \forall z \in \mathcal{Z} \,\, \backslash \,\,\{z_0\}
\]
\[
    \inf_{u \in \mathbb{U}} [L_F V(z) + L_G V(z)u] \leq 0 \quad \forall z \in \mathcal{Z}
\]

where \(L_F V\) and \(L_G V\) are the Lie derivatives of \(V\) along \(F\) and \(G\), respectively. \(\mathcal{Z}\) is the set of all possible augmented states. The CLF will define the set of admissable control gains \(K\) as follows:
\[
    K = \{ u: L_f V(z) + L_g V(z)u \leq 0 \}
\]
