...
\]
where \(o(\delta)\) includes high-order terms that approach zero when \(\delta\) tends to zero, \(\nabla_t \tilde{J}\) and \(\nabla_x \tilde{J}\) (a row vector) denote the partial derivates of \(\tilde{J}\) with respect to \(t\) and \(x\), respectively. Plugging the first-order Taylor expansion into the DP recursion \eqref{eq:ct-optimal-control-discrete-approx-dp}, we obtain
\begin{equation}
...
\hypertarget{convex-function}{%
\subsection{Convex function}\label{convex-function}}
...
\hypertarget{kkt-condition}{%
\subsection{KKT condition}\label{kkt-condition}}
