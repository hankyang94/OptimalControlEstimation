...
In fact, MPC was originally developed in the context of chemical plant control [@borrelli17book-mpc], which has linear system dyanmics, as we will introduce in the following section.
...
\end{equation}
where the state $x_t$ and control $u_t$ are constrained to lie in the following polyheral sets (see Definition \@ref(def:polyhedra))
\begin{split}
...
\begin{equation}
\begin{split}
J_t^\star(x_t) = \min_{u(0),\dots,u(N-1)} & \quad x(N)^T P x(N) + \sum_{k=0}^{N-1} x(k)^T Q x(k) + u(k)^T R u(k) \\
...
1. Terminal cost $P$: in the objective of problem \@ref(eq:linear-mpc-subroutine), there is an additional terminal cost $x(N)^T P x(N)$.
...
We will see that it is crucial to design the terminal cost matrix $P$ and terminal constraint set $\mathcal{X}_f$.

Above all, problem \@ref(eq:linear-mpc-subroutine) is a convex optimization problem that we know how to solve efficiently (you have solved such convex optimization problems in Exercise \@ref(exr:lqrconstraints)).

Let $u^\star(0),\dots,u^\star(N-1)$ be the optimal solution of problem \@ref(eq:linear-mpc-subroutine), the RHS framework will only apply the first control u^\star(0) to the system, and hence the closed-loop system is
$$
...
Let's apply RHS to the double integrator example. We will see that if $P$ and $\mathcal{X}_f$ are not chosen properly, then the subproblem \@ref(eq:linear-mpc-subroutine) does get infeasible.
...
$$
In the RHS subproblem \@ref(eq:linear-mpc-subroutine), we use $N = 3$, $P = Q = I$, $R=10$, and $\mathcal{X}_f = \mathbb{R}^2$ (i.e., there is not terminal constraint).
